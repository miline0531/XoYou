...
import kr.co.genericit.mybase.xoyou2.network.modelxo.ReqXoYouMeetLoad;
...
public class ActionRequestXoYouMeetLoad extends Action {

    public ActionResultListener mActionResultInterface;
    public String mUserId,mDate;

    public ActionRequestXoYouMeetLoad(Activity context, String userId, String date, ActionResultListener actionResultInterface) {
...
//            actionDone("", null, "");
    ...
    public void run() {
        ...
        int keywordCount = 0;
        ReqXoYouMeetLoad data = new ReqXoYouMeetLoad();
        if(mUserId != null) data.setUserId(mUserId);
        if(mDate != null) data.setDate(mDate);

        mRetrofitAPIService.requestXoYouMeetLoad(data).enqueue(mCallback);
    }
}
