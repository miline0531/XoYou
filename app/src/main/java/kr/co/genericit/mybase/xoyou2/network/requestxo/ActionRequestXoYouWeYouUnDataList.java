...
import kr.co.genericit.mybase.xoyou2.network.modelxo.ReqXoYouWeYouUnDataList;
...
public class ActionRequestXoYouWeYouUnDataList extends Action {

    public ActionResultListener mActionResultInterface;
    public String mUserId,mDate,mUnName;

    public ActionRequestXoYouWeYouUnDataList(Activity context, String userId, String date, String unName, ActionResultListener actionResultInterface) {
...
//            actionDone("", null, "");
    ...
    public void run() {
        ...
        int keywordCount = 0;
        ReqXoYouWeYouUnDataList data = new ReqXoYouWeYouUnDataList();
        if(mUserId != null) data.setUserId(mUserId);
        if(mDate != null) data.setDate(mDate);
        if(mUnName != null) data.setUnName(mUnName);

        mRetrofitAPIService.requestXoYouWeYouUnDataList(data).enqueue(mCallback);
    }
}
