...
import kr.co.genericit.mybase.xoyou2.network.modelxo.ReqXoYouUserQASimRiDetail;
...
public class ActionRequestXoYouUserQASimRiDetail extends Action {

    public ActionResultListener mActionResultInterface;
    public String mUserId,mDate,mYuSeq;

    public ActionRequestXoYouUserQASimRiDetail(Activity context, String userId, String yuSeq, String date, ActionResultListener actionResultInterface) {
...
//            actionDone("", null, "");
    ...
    public void run() {
        ...
        int keywordCount = 0;
        ReqXoYouUserQASimRiDetail data = new ReqXoYouUserQASimRiDetail();
        if(mUserId != null) data.setUserId(mUserId);
        if(mYuSeq != null) data.setYuSeq(mYuSeq);
        if(mDate != null) data.setDate(mDate);

        mRetrofitAPIService.requestXoYouUserQASimRiDetail(data).enqueue(mCallback);
    }
}
